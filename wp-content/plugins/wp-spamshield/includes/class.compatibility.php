...
final class WPSS_Compatibility extends WP_SpamShield {
	...
	static public function supported() {
		...
		if( self::is_plugin_active( 'gwolle-gb' ) ) {
			...
			if( empty( $spamshield_options['disable_misc_form_shield'] ) ) {
				...
				if( 'post' === WPSS_Func::lower( $_SERVER['REQUEST_METHOD'] ) && !empty( $_POST ) ) {
					if( is_admin() ) {
	...
	static public function misc_form_bypass() {
		 ...
		 *	@since	1.9.9.9.5
		 */
		if( rs_wpss_is_doing_ajax() && 'post' === WPSS_Func::lower( $_SERVER['REQUEST_METHOD'] ) && !empty( $_POST['action'] ) && 'slimtrack' === $_POST['action'] && self::is_plugin_active( 'wp-slimstat' ) ) {
			return TRUE;
		}
