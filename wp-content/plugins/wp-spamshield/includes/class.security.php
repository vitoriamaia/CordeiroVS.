...
class WPSS_Security extends WP_SpamShield {
	...
	static public function get_raw_post_data() {
		...
		if( !empty( $_SERVER['REQUEST_METHOD'] ) && 'post' === WPSS_Func::lower( $_SERVER['REQUEST_METHOD'] ) ) {
			$_SERVER['X_RAW_POST_DATA'] = $HTTP_RAW_POST_DATA = ( !isset( $HTTP_RAW_POST_DATA ) ) ? @file_get_contents( 'php://input' ) : $HTTP_RAW_POST_DATA;
	...
	static public function check_request_method() {
		...
		if( !empty( $_SERVER['REQUEST_METHOD'] ) && !parent::preg_match( "~^(GET|HEAD|OPTIONS|POST|PUT|PATCH|DELETE)$~iu", $_SERVER['REQUEST_METHOD'] ) ) {
			if( TRUE === WPSS_IP_BAN_ENABLE ) { self::ip_ban(); }
